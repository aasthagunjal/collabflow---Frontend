...
export default function ProjectsView({
  ...
  return (
    <div className="space-y-lg animate-fade-in pb-xl">
      {/* Workspaces & Projects Header Section */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-md">
        ...
        <div className="flex items-center gap-md shrink-0">
          {/* Active completed archived filters tab buttons */}
            ...
            {(['Active', 'Completed', 'Archived'] as const).map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full font-headline text-xs font-semibold shrink-0 cursor-pointer transition-all ${
                    active
                      ? 'bg-white text-primary shadow-sm'
                      : 'text-secondary hover:text-[#4744e5]'
                  }`}
                >
                  {tab}
                </button>
      ...
      {/* Projects Grid (Bento Style & Cards) */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-lg">
        {filteredProjects.map((proj) => {
          // Check if this project is the featured one (Strategy Global Expansion Q4, wide layout)
            ...
            return (
              ...
              >
                <div className="flex-1 flex flex-col gap-lg z-10">
                  ...
                  <div>
                    <h3 className="font-headline font-semibold text-lg text-on-surface group-hover:text-primary transition-colors cursor-pointer">
                      {proj.name}
                    </h3>
                ...
                {/* Right Illustration Backplane Displays representing Screen 5 Featured image */}
                {proj.image && (
                  <div className="w-full md:w-1/3 rounded-lg overflow-hidden relative min-h-[200px] bg-slate-950 shadow border border-outline-variant">
                    <img 
                      ...
                      src={proj.image} 
                      alt="Data dashboard global network" 
                    />
          ...
          return (
            ...
            >
              ...
              <div>
                <h3 className="font-headline font-semibold text-base text-on-surface group-hover:text-primary transition-colors cursor-pointer">
                  {proj.name}
                </h3>
                ...
                <div className="w-full h-2 bg-[#eff4ff] rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full ${
                      proj.status === 'On Hold' ? 'bg-[#ffb68c]' : 'bg-[#4744e5]'
                    }`} 
                    style={{ width: `${proj.progress}%` }} 
                  />
                </div>
