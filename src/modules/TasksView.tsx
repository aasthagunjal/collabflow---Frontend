...
export default function TasksView({
  ...
  const handleProjectChange = (v: string) => { setSelectedProject(v); setCurrentPage(1); };
      ...
      <div className="space-y-lg animate-fade-in pb-xl">
        ...
        <div className="flex justify-between items-end">
          ...
          <button
            ...
            className="bg-[#4744e5] hover:bg-opacity-90 text-white px-lg py-2 rounded-xl flex items-center gap-sm font-headline text-xs font-bold shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer"
          >
                ...
                {isLoading ? (
                  <tr>
                    <td colSpan={8} className="px-lg py-xl text-center">
                      <div className="flex items-center justify-center gap-sm text-secondary">
                        ...
                        <span className="font-headline text-sm">Loading tasks...</span>
                      </div>
                    </td>
                  </tr>
                ) : error ? (
                  <tr>
                    <td colSpan={8} className="px-lg py-xl text-center">
                      <div className="flex flex-col items-center gap-sm">
                        ...
                        <p className="text-sm font-semibold text-on-surface">Failed to load tasks</p>
                  ...
                  : filteredTasks.length === 0 ? (
                    <tr>
                      <td colSpan={8} className="px-lg py-xl text-center">
                        <div className="flex flex-col items-center gap-sm">
                          <ListTodo className="w-8 h-8 text-outline" />
          ...
          <div className="px-lg py-md bg-white border-t border-border-subtle flex items-center justify-between font-headline select-none">
            <p className="text-[11px] font-medium text-on-surface-variant">
              ...
              <span className="font-bold">
                {filteredTasks.length === 0 ? 0 : (currentPage - 1) * PAGE_SIZE + 1}–{Math.min(currentPage * PAGE_SIZE, filteredTasks.length)}
              </span>{' '}
              of <span className="font-bold">{filteredTasks.length}</span> tasks
            </p>
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                ...
                className="p-1 rounded hover:bg-[#eff4ff] text-secondary active:scale-90 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
              >
              ...
              {getPageNumbers().map((p, i) =>
                p === '...' ? (
                  <span key={`ellipsis-${i}`} className="px-1 text-secondary text-[10px]">...</span>
                ) : (
                  <button
                    key={p}
                    onClick={() => setCurrentPage(p as number)}
                    className={`w-6 h-6 rounded font-extrabold text-[10px] flex items-center justify-center shrink-0 transition-all ${currentPage === p
                      ? 'bg-[#4744e5] text-white shadow-sm shadow-[#4744e5]/20'
                      : 'hover:bg-[#eff4ff] text-secondary'
                      }`}
                  >
                    {p}
                  </button>
