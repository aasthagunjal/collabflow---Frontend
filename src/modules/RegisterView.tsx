...
import { registerUser, resolveRegisterError } from "../services/auth/authService";
import {
  registerStart,
  registerSuccess,
  registerFailure,
...
export default function RegisterView({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!firstName.trim() || !lastName.trim()) {
      dispatch(registerFailure("Please enter your first and last name."));
      return;
    }
    if (!email.trim()) {
    ...
    if (!password.trim()) {
      dispatch(registerFailure("Please enter a password."));
      return;
    }
    ...
    try {
      ...
      // Brief pause so the user sees the success message, then go to login
      setTimeout(() => {
        dispatch(setAuthRoute("login"));
        onNavigateToLogin();
      }, 1800);
    } catch (err: any) {
      dispatch(registerFailure(resolveRegisterError(err)));
    }
  };
  ...
  return (
    <div className="min-h-screen bg-bg-slate-50 flex items-center justify-center p-lg font-sans">
      ...
      <div className="w-[360px] bg-white border border-border-subtle rounded-2xl p-lg shadow-xl flex flex-col gap-lg animate-scale-up select-none">
        ...
        {/* Feedback banner — reserved slot to prevent layout shift */}
        <div className="min-h-[36px]">
          {successMsg ? (
            <div className="flex gap-sm p-sm bg-[#d6ffe0] border border-[#86efac] rounded-xl text-xs text-[#15803d]">
              <CheckCircle2 className="w-4 h-4 shrink-0 mt-[1px]" />
              <span className="leading-relaxed">{successMsg}</span>
            </div>
          ) : error ? (
            <div className="flex gap-sm p-sm bg-[#ffdad6] border border-[#ffb4ab] rounded-xl text-xs text-[#ba1a1a]">
        ...
        <div className="grid grid-cols-2 gap-sm select-none">
          <button
            type="button"
            disabled={isLoading}
            className="flex items-center justify-center gap-xs border border-[#c7c4d8] hover:border-primary px-3 py-2 rounded-xl text-xs transition-all bg-white font-semibold cursor-pointer text-on-surface-variant leading-none disabled:opacity-50"
          >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-md select-text">
          ...
          <div className="grid grid-cols-2 gap-sm">
            <div className="space-y-sm">
              ...
              <div className="relative">
                <UserIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-outline pointer-events-none" />
                <input
                  type="text"
                  value={firstName}
                  ...
                  placeholder="Rahul"
                  disabled={isLoading}
                  className="w-full pl-9 pr-2 py-2 bg-[#f8fafc] border border-[#c7c4d8] rounded-xl text-xs outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 transition-all font-medium placeholder:text-outline disabled:opacity-60"
                  required
                />
              </div>
            </div>
            ...
            <div className="space-y-sm">
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="Snow"
                  disabled={isLoading}
                  className="w-full px-3 py-2 bg-[#f8fafc] border border-[#c7c4d8] rounded-xl text-xs outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 transition-all font-medium placeholder:text-outline disabled:opacity-60"
                  required
                />
              </div>
            </div>
          ...
          <div className="flex items-start gap-sm select-none">
            <input
              id="terms"
              type="checkbox"
              className="mt-[2px] w-4 h-4 rounded border-[#c7c4d8] text-primary focus:ring-primary focus:ring-opacity-20 cursor-pointer"
              required
            />
            <label htmlFor="terms" className="text-xs text-[#5e617d] leading-normal cursor-pointer">
              I agree to the{" "}
              <a href="#" className="font-bold text-primary hover:underline">Terms of Service</a>
              {" "}and{" "}
        ...
        <p className="text-xs text-[#5e617d] text-center select-none">
